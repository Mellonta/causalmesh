...
def build_servers():
    # group = server_group()
    # res = group.run(f"cd ~/ccmesh && ~/.cargo/bin/cargo build --release --bin hz-server", hide=True)
    res = []
...
def run_once(rps, warmup=10):
    assert ALG in ["mesh", "con", "opt", "cb", "media"]
    redis = redis_conn()
...
def run_once2(rps, warmup=10):
    ...
    client = custom_conn("Haoran@ms1018.utah.cloudlab.us")
    dura = 60
    if warmup > 0:
        print("Warming up...", end=", ")
        client.run(oha_addr(rps, warmup, "10.10.1.15"), hide=True)
    if ALG == "media":
        time.sleep(5)
    print("Evaluating...", end=", ")
    assert not SCALE_LAMBDA and TOOL == "oha"
    output = client.run(oha_addr(rps, dura, "10.10.1.15"), hide=True)
    res = parse_oha(output.stdout)
    # print(ALG, end=", ")
    # print(rps, end=", ")
    # pprint(res)
